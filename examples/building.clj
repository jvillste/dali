(ns argumentica.example.building
...
(defonce server-state-atom (atom nil #_(server-api/create-state (-> (crud-server/create-in-memory-btree-db 21)
...
(defn crud-client []
  (let [client-db-atom (atom (client-db/create #_(client/->HttpClient "http://localhost:4010/api")
                                               ...
                                               common/base-index-definition #_ crud-server/imdb-index-definition))]
    (fn [width height]
